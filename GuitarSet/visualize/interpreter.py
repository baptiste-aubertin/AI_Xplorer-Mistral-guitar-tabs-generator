...
def tablaturize_jams_ascii(jam):
    str_midi_dict = {0: 40, 1: 45, 2: 50, 3: 55, 4: 59, 5: 64}
    string_dict = {0: 'E', 1: 'A', 2: 'D', 3: 'G', 4: 'B', 5: 'e'}
    tab_dict = {string: [] for string in string_dict.values()}
    ...
    for s, string_tran in enumerate(annos):
        for note in string_tran:
            ...
            midi_note = note[2]
            fret = int(round(midi_note - str_midi_dict[s]))
            tab_dict[string_dict[s]].append((start_time, fret))

    # Sort the notes by time for each string
    for string in tab_dict:
        tab_dict[string] = sorted(tab_dict[string])

    # Find the max time to determine the length of the tab
    max_time = max([note[0] for notes in tab_dict.values() for note in notes])

    # Initialize tab lines
    tab_lines = {string: "" for string in string_dict.values()}
    for string in tab_lines:
        tab_lines[string] = string + " |"

    # Define the time resolution (in seconds) per character
    time_resolution = 0.25
    num_columns = int(max_time / time_resolution) + 1

    # Fill in the tablature
    for string in tab_dict:
        current_pos = 0
        for note in tab_dict[string]:
            note_pos = int(note[0] / time_resolution)
            while current_pos < note_pos:
                tab_lines[string] += "-"
                current_pos += 1
            tab_lines[string] += str(note[1])
            current_pos += 1
        while current_pos < num_columns:
            tab_lines[string] += "-"
            current_pos += 1

    # Join the tab lines into a final tab output
    final_tab = "\n".join(tab_lines[string] for string in string_dict.values())
